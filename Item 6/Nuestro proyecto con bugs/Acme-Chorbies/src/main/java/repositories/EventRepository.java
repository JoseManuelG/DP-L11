...
public interface EventRepository extends JpaRepository<Event, Integer> {
	...
	@Query("select e, e.seatsOffered - count(r) from Register r right join r.event e group by e order by e.organisedMoment")
	...
	@Query("select e, e.seatsOffered - count(r) from Register r right join r.event e where ?2 <= e.organisedMoment and e.organisedMoment <= ?1 and e.seatsOffered > (select count(r2) from Register r2 where r2.event = e) group by e order by e.organisedMoment")
	...
	@Query("select e, e.seatsOffered - count(r) from Register r right join r.event e where e.manager.id = ?1 group by e order by e.organisedMoment")
