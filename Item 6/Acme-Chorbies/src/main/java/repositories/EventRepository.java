...
public interface EventRepository extends JpaRepository<Event, Integer> {
	...
	@Query("select r.event, r.event.seatsOffered - count(r) from Register r group by r.event order by r.event.seatsOffered - count(r)")
	...
	@Query("select r.event, r.event.seatsOffered - count(r) from Register r where ?2 <= r.event.organisedMoment and r.event.organisedMoment <= ?1 and r.event.seatsOffered > (select count(r2) from Register r2 where r2.event = r.event) group by r.event order by r.event.seatsOffered - count(r)")
	...
	@Query("select r.event, r.event.seatsOffered - count(r) from Register r where r.event.manager.id = ?1 group by r.event order by r.event.seatsOffered - count(r)")
